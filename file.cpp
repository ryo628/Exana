...
void outputNode(struct treeNode *node, FILE *fp)
{
  ...
  if(workingSetAnaMode==1){
    ...
    fwrite(&a->maxCntR, sizeof(UINT64), 1, fp);
    fwrite(&a->minCntR, sizeof(UINT64), 1, fp);
    fwrite(&a->sumR, sizeof(UINT64), 1, fp);
    fwrite(&a->maxCntW, sizeof(UINT64), 1, fp);
    fwrite(&a->minCntW, sizeof(UINT64), 1, fp);
    fwrite(&a->sumW, sizeof(UINT64), 1, fp); 
...
void output_gListOfLoop(FILE *fp)
{
  ...
  for(int i=0;i<num_gListOfLoop;i++){
    ...
    string rtnName=(*rtnArray[node->rtnID]->rtnName);
    int s1=rtnName.size();
    const char *c1=(rtnName).c_str();
...
int searchNodeID(struct treeNodeMapTableE *t, UINT64 num, int id)
{
  ...
  cerr<<"Error: cannot find nodeID in NodeMapTable @searchNodeID()"<<endl;
  exit(1);
}
...
void updateTreeNode(struct treeNode *arrayNode, struct treeNodeMapTableE *t, UINT64 num)
{
  ...
  for(UINT i=0;i<num;i++){
	...
	exit(1);
      ...
      //cerr<<"curr1 "<<hex<<curr1<<endl;;//<<"  depNodeID "<<dec<<depList->nodeID<<" ";
      curr1->node=t[(depList->nodeID)].nodeAdr;
	...
	exit(1);
      ...
      //cerr<<"curr2 "<<hex<<curr2<<endl;;//<<"  depNodeID "<<dec<<depList->nodeID<<" ";
      curr2->node=t[(recList->nodeID)].nodeAdr;
      //cerr<<"depList update "<<hex<<curr2->node<<endl;
  ...
  for(UINT64 i=0;i<num;i++){
    ...
    */
	...
	list=list->next;
      ...
      //cerr<<"recNodeList nodeID="<<dec<<nodeID<<endl;
      list->next=NULL;
    }
